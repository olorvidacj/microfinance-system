...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onSuccess }) => {
  ...
  const handleStep1Continue = async () => {
    ...
    // If all Step 1 fields are valid, send OTP to Gmail using Supabase
    setLoading(true);
    try {
      ...
      Alert.alert(
        ...
        `A verification code has been dispatched via Gmail to:\n\n${email.trim()}\n\nPlease check your Gmail inbox (and Spam folder) to enter the code.`
      );
    } catch (err: any) {
      Alert.alert('Verification Dispatch Failed', err.message || 'Unable to send verification code. Please check your email.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // -------------------------------------------------------------
  // Verify Registration OTP (Authentic Supabase Code)
  // -------------------------------------------------------------
  const handleVerifyRegistrationOtp = async () => {
    const cleanCode = regOtp ? regOtp.trim().replace(/\D/g, '') : '';
    if (!cleanCode || cleanCode.length < 6 || cleanCode.length > 8) {
      setStep1Errors((prev) => ({ ...prev, otp: 'Please enter the complete verification code sent to your Gmail' }));
      return;
    }

    setLoading(true);
    try {
      await api.verifyRegistrationOtp(phone.trim(), cleanCode, email.trim());
      setIsPhoneVerified(true);
      setIsVerifyingOtp(false);
      // Advance to Step 2!
      setRegStep(2);
    } catch (err: any) {
      setStep1Errors((prev) => ({ ...prev, otp: err.message || 'Invalid verification code. Please check your Gmail.' }));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.sendRegistrationOtp(phone.trim(), email.trim());
      setRegOtp('');
      setOtpCountdown(60);
      setCanResendOtp(false);
      Alert.alert('New Code Dispatched 📬', `A fresh 6-digit verification code has been sent to ${email.trim()}.\nPlease check your Gmail inbox.`);
    } catch (err: any) {
  ...
  const handleForgotPassword = async () => {
    ...
    try {
      ...
      setRecoveryOtp('');
      setMode('verify_otp');
      Alert.alert('Reset Code Sent 📬', `A 6-digit password reset code has been sent to ${recoveryEmailOrPhone.trim()}.\nPlease check your Gmail inbox.`);
    } catch (err: any) {
      Alert.alert('Error', err.message);
    } finally {
      setLoading(false);
      ...
      <View style={styles.card}>
        ...
        {mode === 'register' ? (
          <View>
            ...
            {/* ------------------------------------------------------------- */}
            {/* STEP 1.5: GMAIL OTP VERIFICATION DIALOG */}
            {/* ------------------------------------------------------------- */}
            {regStep === 1 && isVerifyingOtp && (
              <View style={styles.formSection}>
                  ...
                  <View style={styles.otpTimerRow}>
                    {!canResendOtp ? (
                      ...
                      <TouchableOpacity onPress={handleResendOtp} disabled={loading}>
                        <Text style={styles.resendLink}>📩 Tap here to Resend Code to Gmail</Text>
                      </TouchableOpacity>
                    )}
                  </View>

                  <View style={styles.checkSpamNotice}>
                    <Text style={styles.checkSpamNoticeText}>
                      💡 Check your Gmail app's Inbox and Spam / Promotions tab.
                    </Text>
                  </View>
                </View>
                ...
                >
                  <Text style={styles.editPhoneBtnText}>Edit Email or Phone Number</Text>
                </TouchableOpacity>
              </View>
            )}
