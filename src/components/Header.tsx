      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <button
            type="button"
            onClick={openSidebar}
            className="lg:hidden -ml-1 mr-1 rounded-xl p-2 text-slate-600 transition hover:bg-slate-100"
            aria-label="Open navigation menu"
          >
            <Menu className="w-5 h-5" />
          </button>

          {/* Brand Logo & Name (mobile only; sidebar carries the brand on desktop) */}
          <div className="flex items-center gap-3 lg:hidden">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-gold-500 to-gold-400 flex items-center justify-center text-navy-950 shadow-md font-bold text-xl">
            ...
            {showBranchDropdown && (
              <div className="absolute left-1/2 -translate-x-1/2 mt-2 w-72 bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in zoom-in-95 duration-100">
                <div className="px-3.5 py-2 border-b border-slate-100 text-xs font-semibold text-slate-400 uppercase tracking-wider flex justify-between items-center">
                  ...
                  <span className="text-gold-600 font-normal">{branches.length} Active</span>
                </div>
                <button
                  onClick={() => {
                  ...
                  className={`w-full text-left px-3.5 py-2.5 text-sm flex items-center justify-between transition hover:bg-slate-50 ${
                    activeBranchId === 'all' ? 'bg-gold-500/10 font-semibold text-gold-700' : 'text-slate-700'
                  }`}
                >
                ...
                {branches.map((branch) => (
                  <button
                    ...
                    className={`w-full text-left px-3.5 py-2.5 text-sm flex items-center justify-between transition hover:bg-slate-50 ${
                      activeBranchId === branch.id ? 'bg-gold-500/10 font-semibold text-gold-700' : 'text-slate-700'
                    }`}
                  >
                    <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3.5 py-1.5 bg-gold-400 hover:bg-gold-300 text-navy-950 rounded-xl text-sm font-semibold transition shadow-xs"
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border transition shadow-xs ${
                dbStatus.connected
                  ...
                  : 'bg-gold-500/10 text-gold-700 border-gold-400/30 hover:bg-gold-500/20'
              }`}
            >
              <Database className={`w-3.5 h-3.5 ${dbStatus.connected ? 'text-emerald-600' : 'text-gold-600'}`} />
              <span className="hidden sm:inline">
                {dbStatus.connected ? 'Supabase Connected' : 'Supabase Setup'}
              </span>
              <span className="sm:hidden">DB</span>
              <span className={`w-2 h-2 rounded-full ${dbStatus.connected ? 'bg-emerald-500 animate-pulse' : 'bg-gold-500'}`}></span>
            </button>
          </div>
        </div>
      </div>
